...
public class VooMediator {
	...
	private static final String[] AEROPORTOS = {"GRU", "CGH", "GIG", "SDU", "REC", "CWB", "POA", "BSB", "SSA", "FOR", "MAO", "SLZ", "CNF", "BEL", "JPA", "PNZ", "CAU", "FEN", "SET", "NAT", "PVH", "BVB", "FLN", "AJU", "PMW", "MCZ", "MCP", "VIX", "GYN", "CGB", "CGR", "THE", "RBR", "VCP", "RAO"};
	...
	public String validar(Voo voo) {
		...
		if (StringUtils.isVaziaOuNula(aeOrigem) || !estaNaLista(aeOrigem)) {
			return "Aeroporto origem errado";
		}
		if (StringUtils.isVaziaOuNula(aeDestino) || !estaNaLista(aeDestino)) {
			return "Aeroporto destino errado";
		}
		if (aeOrigem.equals(aeDestino)) {
			return "Aeroporto origem igual a aeroporto destino";
		}
		String msg = validarCiaNumero(voo.getCompanhiaAerea(), voo.getNumeroVoo());
		if (msg != null) {
			return msg;
		}
		DiaDaSemana[] dias = voo.getDiasDaSemana();
		if (dias == null || dias.length == 0) {
			return "Dias da semana nao informados";
		}
		boolean[] diasRepetidos = new boolean[7];
		for (DiaDaSemana dia : dias) {
			if (diasRepetidos[dia.getCodigo() - 1]) {
				return "Dia da semana repetido";
			}
			diasRepetidos[dia.getCodigo() - 1] = true;
		}
		if (voo.getHora() == null) {
